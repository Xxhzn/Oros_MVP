...
#var ablities:SkillData
#var attacktarget:CharacterData
...
#func enter():
	...
	#while 1:
		...
		#while turn_index < round_action_order.size():
			...
			#fsm.change_state(Battle_Scene.States.Win)
			#break
		#elif is_group_died_all(battle_scene.our_team_character):
			#fsm.change_state(Battle_Scene.States.Lose)
			#break
			#
		#conutNum += 1
			#
## 当前单位行动开始时的调试入口
#func on_character_turn_start(character: CharacterData):
...
#
## 当前单位行动结束时的状态结算
#func on_character_turn_end(character: CharacterData):
...
#func delay_target_one_slot_in_current_round(tarGet: CharacterData, round_action_order: Array, turn_index: int):
...
#
		#
#func enemy_start_attack(character:CharacterData):
